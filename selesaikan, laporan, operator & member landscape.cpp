...
void menuLaporan() {
    ...
    garisBatas();
    tengah("Total Pendapatan : Rp " + to_string(totalPendapatan), 105);
    garisBatas();
    ...
    for (int i = 0; i < jumlahBooking; i++) {
        if (daftarBooking[i].status == "Selesai") {
            tengah(to_string(no++) +
                   ...
                   " | Unit " + to_string(daftarBooking[i].unitId) +
                   " | " + to_string(daftarBooking[i].durasi) + " jam" +
                   " | Rp " + to_string(daftarBooking[i].totalHarga), 105);
            ada = true;
        }
    }
...
void menuMemberLandscape() {
    ...
    while (true) {
        ...
        tombol = getch();
        if      (tombol == 75) posisi = (posisi == 0) ? totalMenu - 1 : posisi - 1; 
        else if (tombol == 77) posisi = (posisi == totalMenu - 1) ? 0 : posisi + 1; 
...
void menuOperatorLandscape() {
    ...
    while (true) {
        ...
        if      (tombol == 75) posisi = (posisi == 0) ? totalMenu - 1 : posisi - 1; 
            ...
            currentUsername = ""; currentRole = ""; operatorLoginIdx = -1;
            efekLoading("Logout"); return;
        } else if (tombol == 13) { 
